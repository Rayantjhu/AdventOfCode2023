fun main() {
    /*
        Each scratchcard has winning numbers in the first part, delimited by a vertical bar (|) are the numbers that are
        ...
        owned number if it is in the winning numbers and calculate the cards score
     */
    fun part1(input: List<String>): Int {
        return input.sumOf { card ->
            // Get the amount of winning numbers for the card
    ...
    /*
        This time we need to find the amount of instances for each card. The first instinct is: recursion. For each card
        find all the winning numbers and find its copies and return the amount of copies plus 1 (the original card).
...
/** Gets the amount of winning numbers for the given card. */
...
/** Finds the amount of copies for the given card recursively. */
fun findAmountOfCopies(
    ...
    // If the amount of instances is already calculated, return it
    ...
    if (winningAmount == 0) {
        // Save the amount of instances for this card
        savedCalculations[currentIndex] = 1
        return 1
    }

    // Starting amount is 1 because we also have to count the current card
    var amount = 1
    for (i in 1..winningAmount) {
        val newIndex = currentIndex + i
        // Increment the amount by recursively looking for the amount of copies
