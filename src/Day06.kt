...
private object Day06 {
    /*
    For each race, we have to determine the number of ways we can win the race. Multiply these numbers and we have
    the solution.
    ...
    - Knowing math, the middle (median) amount of time held of the total time will always result in the longest
        ...
        until the distance is lower than the minimum distance required to win the race. We then multiply the number
    ...
    So the number of winning observations is 2 * 2 = 4.
    ...
    one number. We can see the number of observations where the distance is greater than 200 is 4 times. This time
    we double the number of observations and add 1 to get the solution: 4 * 2 + 1 = 9.
 */
    fun part1(input: List<String>): Int {
        ...
        return times.zip(distances).map { (time, distance) ->
            ...
            // Calculate the number of winning observations
